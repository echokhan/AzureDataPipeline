--DROP PROCEDURE uspHourlyUpsertTransformedWeatherData;
CREATE PROCEDURE uspHourlyUpsertTransformedWeatherData
AS
BEGIN
    -- Step 1: Update records where city_name, date_only, and time_only all match
WITH SourceWithRowNumber AS (
    SELECT 
        CAST(CONVERT(DATETIME, LEFT(DATEADD(S, dt, '1970-01-01'), 19), 120) AS DATE) AS date_only,
        CONVERT(VARCHAR(8), CONVERT(DATETIME, LEFT(DATEADD(S, dt, '1970-01-01'), 19), 120), 108) AS time_only,
        d.city_name,
        d.lat AS latitude, --using from dim (API result (hr) rounds to 4 precision)
        d.lon AS longitude, --using from dim (API result (hr) rounds to 4 precision)
        temp AS temperature,
        visibility,
        dew_point,
        feels_like,
    ...
    FROM 
        [raw].[Hourly_WeatherData] hr 
		INNER JOIN [dim].[city] d
		ON hr.lat = ROUND(d.lat, 4) AND hr.lon = ROUND(d.lon, 4) --API result (hr) rounds to 4 precision resulting in difference
)
